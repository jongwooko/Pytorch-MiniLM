...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, last_layer_or_is_student):
        _self_output = self.self(input_tensor, attention_mask, 
                                 last_layer_or_is_student=last_layer_or_is_student)
        if not last_layer_or_is_student:
            self_output = _self_output
        else:
            self_output, query_layer, key_layer, value_layer = _self_output
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, is_student):
        ...
        all_encoder_layers.append(hidden_states)
        return all_encoder_layers, all_query_layers, all_key_layers, all_value_layers
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                output_all_encoded_layers=True, is_student=False):
        ...
        encoded_layers, query_layers, key_layers, value_layers = self.encoder(embedding_output,
                                                  extended_attention_mask, is_student=is_student)

        pooled_output = self.pooler(encoded_layers)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, pooled_output, query_layers, key_layers, value_layers
...
class MiniLMForPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        super(MiniLMForPreTraining, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                masked_lm_labels=None, next_sentence_label=None, is_student=False):
        sequence_output, pooled_output, query_layers, key_layers, value_layers = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, is_student=is_student)
        prediction_scores, seq_relationship_score = self.cls(
            sequence_output, pooled_output)
        return prediction_scores, seq_relationship_score, query_layers, key_layers, value_layers
...
class MiniLMForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None):
        sequence_output, pooled_output, query_layers, key_layers, value_layers = \
            self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True)
